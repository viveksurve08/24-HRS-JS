...
// We just need to know that an inner function has always access to the variable of the outer (parent) function. That's it.
...
// It might be beneficial to clearify that the call stack is simply the execution order and it is not the same thing as the computer's memory, which is where the variables are held. So even the function was removed from the call stack, its variables are still maintained in memory untill they are no longer needed and garbage collection comes and removes them.
